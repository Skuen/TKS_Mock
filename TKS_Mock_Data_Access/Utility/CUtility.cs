...
namespace TKS_Mock_Data_Access.Utility
{
    public class CUtility
    {
        ...
        {
            if (p_objData != DBNull.Value && Convert_To_String(p_objData) != "")
...
#pragma warning restore SYSLIB0021 // Type or member is obsolete
            ...
            md5.ComputeHash(Encoding.ASCII.GetBytes(p_strText));
    ...
    "ý","ỳ","ỷ","ỹ","ỵ",};
        ...
        public static void Clone_Entity(object p_objSource, object p_objTarget)
        {
            ...
            for (int v_i = 0; v_i < v_objSourceProprties.Length; v_i++)
            {
                ...
                if (v_objSourceProp.CanWrite == true)
                {
                    ...
                    switch (v_strTypedata)
                    {
                        case "String": v_objTargetProp.SetValue(p_objTarget, Convert_To_String(v_objSourceVal)); break;
                        case "Int16": v_objTargetProp.SetValue(p_objTarget, Convert_To_Int32(v_objSourceVal), null); break;
                        case "Int32": v_objTargetProp.SetValue(p_objTarget, Convert_To_Int32(v_objSourceVal), null); break;
                        case "Int64": v_objTargetProp.SetValue(p_objTarget, Convert_To_Int64(v_objSourceVal), null); break;
                        case "DateTime": v_objTargetProp.SetValue(p_objTarget, Convert_To_DateTime(v_objSourceVal), null); break;
                        case "DateTime?": v_objTargetProp.SetValue(p_objTarget, Convert_To_DateTime(v_objSourceVal), null); break;
                        case "Double": v_objTargetProp.SetValue(p_objTarget, Convert_To_Double(v_objSourceVal), null); break;
                        case "Decimal": v_objTargetProp.SetValue(p_objTarget, Convert_To_Double(v_objSourceVal), null); break;
                        case "Boolean": v_objTargetProp.SetValue(p_objTarget, Convert_To_Bool(v_objSourceVal), null); break;
        ...
        public static string Compare_Entity(object p_objSource, object p_objTarget)
        {
            ...
            for (int v_i = 0; v_i < v_objSourceProprties.Length; v_i++)
            {
                    ...
                    && v_strField_Name.ToLower() != "last_updated_by_function")
                {
                    ...
                    if (Convert_To_String(v_objSourceVal).ToLower() != Convert_To_String(v_objTargetVal).ToLower())
                        v_strRes += ", " + v_strField_Name + ": " + Convert_To_String(v_objSourceVal) + " --> " + Convert_To_String(v_objTargetVal);
                }
            }
        ...
        public static string Get_Label_Text(object p_objData, bool p_bOnly_String)
        {
            ...
            if (p_bOnly_String == false)
            {
                switch (v_strType.ToLower())
                {
                    ...
                    case "int32": v_strRes = Convert_To_Int32(p_objData).ToString(CConfig.Number_Format_String); break;
                    case "int64": v_strRes = Convert_To_Int64(p_objData).ToString(CConfig.Number_Format_String); break;
                    case "double": v_strRes = Convert_To_Double(p_objData).ToString("###,###0.#####;-###,###0.#####;-"); break;
                    case "float": v_strRes = Convert_To_Double(p_objData).ToString("###,###0.#####;-###,###0.#####;-"); break;
                    case "datetime":
                        v_strRes = Get_DateTime_Text(Convert_To_DateTime(p_objData));
                        if (v_strRes.EndsWith(" 00:00") == true || v_strRes.EndsWith(" 00:00:00") == true)
                            v_strRes = Get_Date_Text(Convert_To_DateTime(p_objData));
                        break;
                    case "bool":
            ...
            {
                v_strRes = Convert_To_String(p_objData);
            }
        ...
        private static string Get_String_For_Tao_Key(object p_objData)
        {
            ...
            switch (v_strType.ToLower())
            {
                ...
                case "datetime":
                    v_strRes = Get_Date_Text(Convert_To_DateTime(p_objData), "dd-MM-yyyy");
                    break;
                case "bool":
                    if (Convert_To_Bool(p_objData) == true)
                        v_strRes = "1";
        ...
        {
            return File.ReadAllLines(p_strFile);
        }

        public static void Write_File_TXT(string p_strFile, string p_strData)
        {
            File.WriteAllText(p_strFile, p_strData);
        }
